    ...
    >
      ...
      <el-table-column prop="isAssociated" label="关联状态" width="100">
        <template slot-scope="scope">
          <el-tag disable-transitions>{{ relationStatusFormat(scope.row) }}</el-tag>
        </template>
      </el-table-column>
      ...
      <!-- 操作列 (仅在非选择模式下显示) -->
      <el-table-column v-if="!selectionMode" label="操作" width="60" align="center">
        <template slot-scope="scope">
...
<script>
import { listMedia, getEnforcementTypeTree } from '@/api/evidence/evidence_manage_query_api'
