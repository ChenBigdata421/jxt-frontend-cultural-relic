          ...
          <el-table-column prop="permission" label="权限标识" :show-overflow-tooltip="true">
            <template slot-scope="scope">
              <el-popover v-if="scope.row.sysApi.length>0" trigger="hover" placement="top">
                ...
                <!--在 el-popover 组件中，slot="reference" 的内容将作为触发弹出框的元素。当用户与这个元素交互（例如鼠标悬停或点击）时，弹出框会显示出来。-->
          ...
          <div class="demo-drawer__content">
            <el-form ref="form" :model="form" :rules="rules" label-position="top" label-width="106px">
              <el-row>
                ...
                <el-col :span="12">
                  <!--如果不设置 prop，则无法利用 Element UI 的表单验证功能来检查 form.sort 的有效性。因此，保留 prop 是为了确保表单的完整性和验证功能。-->
                ...
                <el-col :span="24">
                  <el-form-item v-if="form.menuType == 'F' || form.menuType == 'C'">
                    ...
                    <!--v-model="form.apis" 用于存储用户在 el-transfer 组件中选择的 API 权限数据。具体来说，它是一个数组，包含用户选择的 API 的 ID。-->
                    <!--props中的key和label的值，分别来自sysapiIdTitleList数组中的对象的id和title字段-->
                    <el-transfer
                      v-model="form.apis"
...
export default {
  ...
  methods: {
    /* handleChange(value, direction, movedKeys) { // direction=right，movedKeys表示新授权的api id的集合;value是原有授权ID与新授权ID的集合
      if (direction === 'right') {
        for (let x = 0; x < movedKeys.length; x++) {
          for (let index = 0; index < this.sysapiList.length; index++) {
            ...
            if (element.id === movedKeys[x]) {
              this.form.sysApi.push(element)
              break
            }
          }
        }
      } else if (direction === 'left') { // direction=left，movedKeys表示被回收的api id的集合
        this.form.sysApi = this.form.sysApi.filter(item => !movedKeys.includes(item.id))
      }
    },*/
    getApiList() {
      this.loading = true
    ...
    normalizer(node) {
      if (node.children && !node.children.length) {
        delete node.children// delete 不是一个方法，而是 JavaScript 的一个操作符。它用于删除对象的属性。
      }
      return {
        id: node.menuId,
        label: node.title,
    ...
    /** 菜单下拉树结构,用于修改和新增菜单项时，指定上级菜单，getRoutes返回的树结构，不包含按钮菜单，因为按钮菜单不能做父菜单项  */
    getTreeselect() {
    ...
    visibleFormat(row) {
      ...
      return this.selectDictLabel(this.visibleOptions, row.visible) // 返回value对应的label，
    },
    // 取消按钮
    cancel() {
      this.open = false
    ...
    findMenuTypeById(objects, id) {
      ...
      function search(objects) {
        for (const obj of objects) {
          ...
          // 如果当前对象有子对象数组，则递归搜索它们
          if (obj.children && obj.children.length > 0) {
            const result = search(obj.children)
            if (result !== undefined) {
              // 如果在子对象数组中找到匹配项，则返回其type
              return result
            }
          }
        }
    ...
    // 判断菜单项是否有子菜单,删除菜单项时，需要检验，不允许删除有子菜单的菜单项
    hasChild(objects, id) {
      // 定义一个递归函数来搜索对象
      function search(objects) {
        for (const obj of objects) {
    ...
    handleUpdate(row) {
      ...
      getMenu(row.menuId).then(response => {
        this.form = response.data// 此时后端会返回向该菜单已授权的api id集合，存在在apis数组中；但在返回所有菜单列表时，各个菜单项是不携带api id的集合，也即apis数组为空。但会携带所有api的全信息。
        this.open = true
        this.isAdd = false
        this.title = '修改菜单'
      })
