<template>
  <BasicLayout>
    <template #wrapper>
      <el-card class="box-card">
        <!-- 查询栏组件 -->
        <DictDataQueryBar
          ref="queryBar"
          :status-options="statusOptions"
          @search="handleSearch"
          @reset="handleReset"
        />

        <!-- 批量操作栏 -->
        <BatchActionBar
          :selected-count="selectedDictDataRecords.length"
          :is-indeterminate="isSelectionIndeterminate"
          :all-selected="isAllSelected"
          @select-all-change="handleSelectAll"
        />

        <!-- 主操作栏 -->
        <div class="main-action-bar">
          <div class="left-actions">
            <el-button
              v-permisaction="['admin:sysDictData:add']"
              type="primary"
              icon="el-icon-plus"
              size="small"
              @click="handleAdd"
            >
              新增字典数据
            </el-button>
            <el-button
              icon="el-icon-refresh"
              size="small"
              type="text"
              class="action-btn tertiary"
              @click="handleRefresh"
            >
              刷新
            </el-button>
            <el-button
              v-permisaction="['admin:sysDictData:export']"
              icon="el-icon-download"
              size="small"
              class="action-btn secondary"
              @click="handleExport"
            >
              导出
            </el-button>
            <el-button
              v-permisaction="['admin:sysDictData:remove']"
              icon="el-icon-delete"
              size="small"
              class="action-btn tertiary-danger"
              :disabled="selectedDictDataRecords.length === 0"
              @click="handleDelete"
            >
              删除
            </el-button>
          </div>
        </div>

        <!-- 字典数据列表 -->
        <el-table
          ref="dictDataTable"
          v-loading="loading"
          :data="dataList"
          border
          @selection-change="handleSelectionChange"
          @sort-change="handleSortChange"
        >
          <el-table-column type="selection" width="60" align="center" />
          <el-table-column
            label="操作"
            align="center"
            class-name="small-padding fixed-width"
            width="180"
            fixed="left"
          >
            <template slot-scope="scope">
              <div class="action-buttons">
                <el-button
                  v-permisaction="['admin:sysDictData:edit']"
                  size="small"
                  type="text"
                  icon="el-icon-edit"
                  class="action-btn tertiary"
                  @click="handleUpdate(scope.row)"
                >
                  修改
                </el-button>
                <el-button
                  v-permisaction="['admin:sysDictData:remove']"
                  size="small"
                  type="text"
                  icon="el-icon-delete"
                  class="action-btn tertiary-danger"
                  @click="handleDelete(scope.row)"
                >
                  删除
                </el-button>
              </div>
            </template>
          </el-table-column>
          <el-table-column
            label="字典编码"
            align="center"
            prop="dictCode"
            sortable="custom"
            width="100"
          />
          <el-table-column
            label="字典标签"
            align="center"
            prop="dictLabel"
            sortable="custom"
          />
          <el-table-column
            label="字典键值"
            align="center"
            prop="dictValue"
            sortable="custom"
          />
          <el-table-column
            label="字典排序"
            align="center"
            prop="dictSort"
            sortable="custom"
            width="100"
          />
          <el-table-column
            label="状态"
            align="center"
            prop="status"
            sortable="custom"
            width="100"
          >
            <template slot-scope="{ row }">
              <el-tag
                :type="row.status === 2 ? 'success' : 'info'"
                size="small"
                effect="dark"
              >
                {{ selectDictLabel(statusOptions, row.status) }}
              </el-tag>
            </template>
          </el-table-column>
          <el-table-column
            label="备注"
            align="center"
            prop="remark"
            :show-overflow-tooltip="true"
          />
          <el-table-column
            label="创建时间"
            align="center"
            prop="createdAt"
            width="180"
            sortable="custom"
          >
            <template slot-scope="scope">
              <span>{{ parseTime(scope.row.createdAt) }}</span>
            </template>
          </el-table-column>
        </el-table>

        <!-- 分页 -->
        <pagination
          v-show="total > 0"
          :total="total"
          :page.sync="queryParams.pageIndex"
          :limit.sync="queryParams.pageSize"
          @pagination="getList"
        />
      </el-card>

      <!-- 新增/修改对话框 -->
      <el-dialog
        :title="title"
        :visible.sync="open"
        width="600px"
        append-to-body
        :close-on-click-modal="false"
        custom-class="edit-dialog"
      >
        <el-form ref="form" :model="form" :rules="rules" label-width="100px">

          <!-- 使用 el-collapse 实现可折叠分组 -->
          <el-collapse v-model="activeFormSections" class="form-collapse">

            <!-- 基础信息 -->
            <el-collapse-item name="basic" class="form-section">
              <template slot="title">
                <div class="section-header">
                  <i class="el-icon-document section-icon" />
                  <span class="section-title">基础信息</span>
                  <span class="section-badge">4项</span>
                </div>
              </template>

              <el-row :gutter="20">
                <el-col :span="12">
                  <el-form-item label="字典类型">
                    <el-input v-model="form.dictType" :disabled="true" />
                  </el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item label="数据标签" prop="dictLabel">
                    <el-input
                      v-model="form.dictLabel"
                      placeholder="请输入数据标签"
                      :disabled="isEdit"
                    />
                  </el-form-item>
                </el-col>
              </el-row>
              <el-row :gutter="20">
                <el-col :span="12">
                  <el-form-item label="数据键值" prop="dictValue">
                    <el-input-number
                      v-model="form.dictValue"
                      placeholder="请输入数据键值"
                      :disabled="isEdit"
                      controls-position="right"
                    />
                  </el-form-item>
                </el-col>
                <el-col :span="12">
                  <el-form-item label="显示排序" prop="dictSort">
                    <el-input-number
                      v-model="form.dictSort"
                      controls-position="right"
                      :min="0"
                    />
                  </el-form-item>
                </el-col>
              </el-row>
            </el-collapse-item>

            <!-- 其他信息 -->
            <el-collapse-item name="other" class="form-section">
              <template slot="title">
                <div class="section-header">
                  <i class="el-icon-info section-icon" />
                  <span class="section-title">其他信息</span>
                  <span class="section-badge">2项</span>
                </div>
              </template>

              <el-row :gutter="20">
                <el-col :span="12">
                  <el-form-item label="状态" prop="status">
                    <el-radio-group v-model="form.status">
                      <el-radio
                        v-for="dict in statusOptions"
                        :key="dict.value"
                        :label="dict.value"
                      >
                        {{ dict.label }}
                      </el-radio>
                    </el-radio-group>
                  </el-form-item>
                </el-col>
                <el-col :span="24">
                  <el-form-item label="备注" prop="remark">
                    <el-input
                      v-model="form.remark"
                      type="textarea"
                      :rows="3"
                      placeholder="请输入备注"
                    />
                  </el-form-item>
                </el-col>
              </el-row>
            </el-collapse-item>

          </el-collapse>
        </el-form>

        <div slot="footer" class="dialog-footer">
          <el-button type="text" class="action-btn tertiary" size="small" @click="cancel">取 消</el-button>
          <el-button type="primary" size="small" @click="submitForm">确 定</el-button>
        </div>
      </el-dialog>
    </template>
  </BasicLayout>
</template>

<script>
import BasicLayout from '@/layout/BasicLayout'
import Pagination from '@/components/Pagination'
import DictDataQueryBar from '@/components/DictDataQueryBar/index.vue'
import BatchActionBar from '@/components/BatchActionBar/index.vue'
import {
  listData,
  getData,
  delData,
  addData,
  updateData
} from '@/api/admin/dict/data'
import { listType, getType } from '@/api/admin/dict/type'
import { formatJson } from '@/utils'

export default {
  name: 'SysDictDataManage',
  components: {
    BasicLayout,
    Pagination,
    DictDataQueryBar,
    BatchActionBar
  },
  data() {
    return {
      // 遮罩层
      loading: true,
      // 总条数
      total: 0,
      // 字典表格数据
      dataList: [],
      // 默认字典类型
      defaultDictType: '',
      // 弹出层标题
      title: '',
      isEdit: false,
      // 是否显示弹出层
      open: false,
      // 表单折叠状态
      activeFormSections: ['basic', 'other'],
      // 状态数据字典
      statusOptions: [],
      // 类型数据字典
      typeOptions: [],
      // 使用 Map 存储所有选中的项（跨分页）
      selectedDictDataMap: {},
      // 防止恢复选中时触发事件循环
      isRestoringSelection: false,
      // 所有选中的字典数据记录
      selectedDictDataRecords: [],
      // 全选状态
      isAllSelected: false,
      isSelectionIndeterminate: false,
      // 查询参数
      queryParams: {
        pageIndex: 1,
        pageSize: 10,
        dictType: undefined,
        dictLabel: undefined,
        status: undefined
      },
      // 表单参数
      form: {},
      // 表单校验
      rules: {
        dictLabel: [
          { required: true, message: '数据标签不能为空', trigger: 'blur' }
        ],
        dictValue: [
          { required: true, message: '数据键值不能为空', trigger: 'blur' }
        ],
        dictSort: [
          { required: true, message: '数据顺序不能为空', trigger: 'blur' }
        ]
      },
      processingInstance: null,
      previousCursor: null
    }
  },
  created() {
    const dictId = this.$route.params && this.$route.params.dictId
    this.getType(dictId)
    this.getTypeList()
    this.getDicts('sys_normal_disable').then((response) => {
      this.statusOptions = response.data.map((item) => ({
        ...item,
        value: parseInt(item.value, 10)
      }))
    })
  },
  methods: {
    /** 查询字典类型详细 */
    getType(dictId) {
      getType(dictId).then((response) => {
        this.queryParams.dictType = response.data.dictType
        this.defaultDictType = response.data.dictType
        this.getList()
      })
    },
    /** 查询字典类型列表 */
    getTypeList() {
      listType({ pageSize: 1000 }).then((response) => {
        this.typeOptions = response.data.list
      })
    },
    /** 查询字典数据列表 */
    getList() {
      this.loading = true
      listData(this.queryParams).then((response) => {
        if (response.code === 200 && response.data) {
          this.dataList = response.data.list.map((item) => ({
            ...item,
            status: Number(item.status)
          }))
          this.total = response.data.count || 0
          // 分页/查询后回显跨分页选择
          this.restoreSelection()
        } else {
          this.dataList = []
          this.total = 0
          this.msgError(response.msg || '获取字典数据列表失败')
        }
      })
        .catch((error) => {
          this.msgError('查询字典数据列表失败：' + (error.message || '未知错误'))
          this.dataList = []
          this.total = 0
        })
        .finally(() => {
          this.loading = false
        })
    },

    /** 恢复选中状态 */
    restoreSelection() {
      if (this.isRestoringSelection) return
      if (!this.$refs.dictDataTable) return
      if (!this.dataList || !this.dataList.length) return

      this.isRestoringSelection = true
      this.$nextTick(() => {
        try {
          this.dataList.forEach((row) => {
            const dictCode = row && row.dictCode
            if (!dictCode) return
            if (this.selectedDictDataMap[dictCode]) {
              this.$refs.dictDataTable.toggleRowSelection(row, true)
            }
          })
        } finally {
          // 使用 setTimeout 确保 toggleRowSelection 的事件处理完成
          setTimeout(() => {
            this.isRestoringSelection = false
          }, 0)
        }
      })
    },

    /** 开始执行操作 */
    startProcessing(text) {
      this.processingInstance = this.$loading({
        lock: true,
        text: text,
        spinner: 'el-icon-loading',
        background: 'rgba(0, 0, 0, 0.3)'
      })
      this.previousCursor = document.body.style.cursor
      document.body.style.cursor = 'wait'
    },

    /** 停止执行操作 */
    stopProcessing() {
      if (this.processingInstance) {
        this.processingInstance.close()
        this.processingInstance = null
      }
      document.body.style.cursor = this.previousCursor
    },

    /** 刷新列表 */
    handleRefresh() {
      this.getList()
    },

    /** 排序回调函数 */
    handleSortChange(column, prop, order) {
      prop = column.prop
      order = column.order
      if (order === 'descending') {
        this.queryParams[prop + 'Order'] = 'desc'
      } else if (order === 'ascending') {
        this.queryParams[prop + 'Order'] = 'asc'
      } else {
        this.queryParams[prop + 'Order'] = undefined
      }
      this.getList()
    },

    /** 多选框选中数据 */
    handleSelectionChange(selection) {
      if (this.isRestoringSelection) {
        return
      }
      const selectedIdSet = new Set(
        (selection || []).map((item) => item && item.dictCode).filter(Boolean)
      )

      ;(this.dataList || []).forEach((row) => {
        const dictCode = row && row.dictCode
        if (!dictCode) return
        if (selectedIdSet.has(dictCode)) {
          this.selectedDictDataMap[dictCode] = row
        } else {
          delete this.selectedDictDataMap[dictCode]
        }
      })

      // 使用 Object.values 确保响应式更新
      this.selectedDictDataRecords = Object.values(this.selectedDictDataMap).filter(Boolean)

      // 更新全选状态
      const totalCount = this.dataList.length
      const selectedCount = this.selectedDictDataRecords.length
      this.isAllSelected = selectedCount === totalCount && totalCount > 0
      this.isSelectionIndeterminate = selectedCount > 0 && selectedCount < totalCount
    },

    /** 批量全选/取消全选 */
    handleSelectAll(val) {
      this.isAllSelected = val
      this.isSelectionIndeterminate = false
      this.$refs.dictDataTable.toggleAllSelection()
    },

    /** 重置按钮操作 */
    resetSelected() {
      this.selectedDictDataMap = {}
      this.selectedDictDataRecords = []
    },

    /** 重置分页 */
    resetPage() {
      this.queryParams.pageIndex = 1
      this.queryParams.pageSize = 10
    },

    /** 查询栏相关方法 */
    handleSearch(searchData) {
      // 合并新的搜索条件
      Object.keys(searchData).forEach(key => {
        this.queryParams[key] = searchData[key]
      })

      this.resetPage()
      this.resetSelected()
      this.getList()
    },

    handleReset() {
      this.queryParams = {
        pageIndex: 1,
        pageSize: 10,
        dictType: this.defaultDictType,
        dictLabel: undefined,
        status: undefined
      }
      this.resetPage()
      this.resetSelected()
      this.getList()
    },

    /** 表单重置 */
    reset() {
      this.form = {
        dictCode: undefined,
        dictLabel: undefined,
        dictValue: undefined,
        dictSort: 0,
        status: 2,
        remark: undefined
      }
      this.resetForm('form')
    },

    /** 取消按钮 */
    cancel() {
      this.open = false
      this.reset()
    },

    /** 新增按钮操作 */
    handleAdd() {
      this.reset()
      this.open = true
      this.title = '添加字典数据'
      this.isEdit = false
      this.form.dictType = this.queryParams.dictType
    },

    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset()
      if (row && row.dictCode !== undefined) {
        getData(row.dictCode).then((response) => {
          if (response.code === 200) {
            this.form = response.data
            this.form.status = Number(this.form.status)
            this.title = '修改字典数据'
            this.isEdit = true
            this.open = true
          } else {
            this.msgError(response.msg || '获取字典数据信息失败')
          }
        }).catch((error) => {
          this.msgError('获取字典数据信息失败：' + (error.message || '未知错误'))
        })
      } else {
        this.form = this.selectedDictDataRecords[0] ? { ...this.selectedDictDataRecords[0] } : {}
        this.form.status = Number(this.form.status)
        this.title = '修改字典数据'
        this.isEdit = true
        this.open = true
      }
    },

    /** 提交按钮 */
    submitForm() {
      this.$refs['form'].validate((valid) => {
        if (valid) {
          if (this.form.dictCode != null) {
            this.startProcessing('正在修改字典数据...')
            updateData(this.form)
              .then(async(response) => {
                if (response.code === 200) {
                  await this.delay(2000)
                  this.resetSelected()
                  this.getList()
                  this.msgSuccess(response.msg || '修改字典数据成功')
                  this.open = false
                } else {
                  this.msgError(response.msg || '修改字典数据失败')
                }
              })
              .catch((error) => {
                this.msgError('修改字典数据失败：' + (error.message || '未知错误'))
              })
              .finally(() => {
                this.stopProcessing()
              })
          } else {
            this.startProcessing('正在创建字典数据...')
            addData(this.form)
              .then(async(response) => {
                if (response.code === 200) {
                  await this.delay(2000)
                  this.getList()
                  this.msgSuccess(response.msg || '新增字典数据成功')
                  this.open = false
                } else {
                  this.msgError(response.msg || '新增字典数据失败')
                }
              })
              .catch((error) => {
                this.msgError('新增字典数据失败：' + (error.message || '未知错误'))
              })
              .finally(() => {
                this.stopProcessing()
              })
          }
        }
      })
    },

    /** 删除按钮操作 */
    async handleDelete(row) {
      try {
        var dictCodes
        var dictLabels
        if (row && row.dictCode !== undefined) {
          dictCodes = [row.dictCode]
          dictLabels = row.dictLabel
        } else {
          dictCodes = this.selectedDictDataRecords.map((item) => item.dictCode)
          dictLabels = this.selectedDictDataRecords.map((item) => item.dictLabel)
        }

        const count = Array.isArray(dictCodes) ? dictCodes.length : 1
        const confirmMessage = count > 1
          ? `是否确认删除选中的 ${count} 条字典数据记录？此操作不可恢复。`
          : `是否确认删除字典数据"${dictLabels}"？此操作不可恢复。`

        await this.$confirm(confirmMessage, '确认删除', {
          confirmButtonText: '删除',
          cancelButtonText: '取消',
          type: 'warning'
        })

        this.startProcessing('正在删除字典数据...')
        const response = await delData({ ids: dictCodes })
        if (response.code === 200) {
          await this.delay(2000)
          this.resetPage()
          this.resetSelected()
          this.getList()
          this.msgSuccess(response.msg || '删除字典数据成功')
        } else {
          this.msgError(response.msg || '删除字典数据失败')
        }
        this.stopProcessing()
      } catch (error) {
        if (error !== 'cancel') {
          this.msgError('删除失败：' + (error.message || '未知错误'))
        }
      }
    },

    /** 延迟函数 */
    delay(ms) {
      return new Promise((resolve) => setTimeout(resolve, ms))
    },

    /** 导出按钮操作 */
    async handleExport() {
      try {
        const hasSelection =
          Array.isArray(this.selectedDictDataRecords) &&
          this.selectedDictDataRecords.length > 0

        const count = hasSelection ? this.selectedDictDataRecords.length : 0
        const confirmText = hasSelection
          ? `是否确认导出已勾选的 ${count} 条字典数据？`
          : '是否确认导出所有字典数据项？'

        await this.$confirm(confirmText, '导出确认', {
          confirmButtonText: '导出',
          cancelButtonText: '取消',
          type: 'info'
        })

        const tHeader = ['字典编码', '字典标签', '字典键值', '字典排序', '状态', '备注']
        const filterVal = ['dictCode', 'dictLabel', 'dictValue', 'dictSort', 'status', 'remark']

        let list = []
        if (hasSelection) {
          list = this.selectedDictDataRecords
        } else {
          const baseQueryParams = { ...(this.queryParams || {}) }

          const pageSize = 1000
          let pageIndex = 1
          let total = Infinity

          while (list.length < total) {
            const query = {
              ...baseQueryParams,
              pageIndex,
              pageSize
            }
            const resp = await listData(query)
            if (!resp || resp.code !== 200) {
              throw new Error((resp && resp.msg) || '查询字典数据列表失败')
            }

            const pageList = (resp.data && resp.data.list) || []
            total = (resp.data && resp.data.count) || 0
            list = list.concat(pageList)

            if (!pageList.length) {
              break
            }
            pageIndex += 1
          }
        }

        const normalizeList = (Array.isArray(list) ? list : []).map((row) => {
          const output = { ...row }
          output.status = this.selectDictLabel(this.statusOptions, row.status)
          output.createdAt = this.parseTime(row.createdAt)
          return output
        })

        const data = formatJson(filterVal, normalizeList)

        // 触发导出
        const excel = await import('@/vendor/Export2Excel')
        excel.export_json_to_excel({
          header: tHeader,
          data,
          filename: '字典数据列表',
          autoWidth: true,
          bookType: 'xlsx'
        })
      } catch (error) {
        if (error !== 'cancel') {
          this.msgError('导出失败：' + (error.message || '未知错误'))
        }
      }
    }
  }
}
</script>

<!--
  样式说明：本页面全部使用全局样式
  全局样式位置：
  - src/styles/index.scss: .filter-container
  - src/styles/components/search.scss: .search-section, .quick-search-form, .search-row, .search-item
  - src/styles/components/dialogs.scss: .edit-dialog, .form-collapse
  - src/styles/components/forms.scss: .section-header, .section-descriptions
  - src/styles/components/buttons.scss: .action-btn, .search-action-buttons
-->
