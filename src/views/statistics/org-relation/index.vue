<template>
  <div class="org-relation-container">
    <!-- 页面头部：筛选条件 -->
    <div class="org-relation__header">
      <h2 class="org-relation__title">机构追加统计</h2>
      <div class="org-relation__filters">
        <!-- 时间筛选 -->
        <el-date-picker
          v-model="timeRange"
          type="daterange"
        ...
        <treeselect
          ...
          class="org-relation__org-select"
        />
        <el-button
          type="primary"
          icon="el-icon-search"
    ...
    <!-- 指标控制栏 -->
    <div class="org-relation__controls">
...
import { getOrgRelationStats } from '@/api/evidence/statistics'
...
export default {
  ...
  methods: {
    ...
    handleReset() {
      ...
      this.chartData = { orgNames: [], metric: 'task_relation_rate', metricLabel: '任务追加率', series: [] }
      this.error = false
      this.fetchData()
    },
...
.org-relation-container {
  padding: 12px;
  background-color: $law-bg-page;
  min-height: calc(100vh - 84px);
}

.org-relation__header {
  display: flex;
  align-items: center;
...
.org-relation__org-select {
  width: 260px;
  min-width: 180px;
}

.org-relation__controls {
  display: flex;
  align-items: center;
  padding: 12px 20px;
  background: $law-bg-card;
