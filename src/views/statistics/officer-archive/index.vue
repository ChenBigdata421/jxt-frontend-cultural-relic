<template>
  <div class="officer-archive-container">
    <!-- 页面头部：筛选条件 -->
    <div class="officer-archive__header">
      <h2 class="officer-archive__title">人员归档统计</h2>
      <div class="officer-archive__filters">
        <!-- 时间筛选 -->
        <el-date-picker
          v-model="timeRange"
          type="daterange"
        ...
        <treeselect
          ...
          class="officer-archive__org-select"
        />
        <!-- 人员筛选 -->
        <el-select
          v-model="policeIds"
          ...
          size="small"
          class="officer-archive__police-select"
        >
          <el-option
    ...
    <!-- 指标控制栏 -->
    <div class="officer-archive__controls">
...
import { getOfficerArchiveStats } from '@/api/evidence/statistics'
...
export default {
  ...
  methods: {
    ...
    handleReset() {
      ...
      this.chartData = { officerNames: [], metric: 'archive_rate', metricLabel: '归档率', series: [] }
      this.error = false
      this.fetchData()
    },
...
.officer-archive-container {
  padding: 12px;
  background-color: $law-bg-page;
  min-height: calc(100vh - 84px);
}

.officer-archive__header {
  display: flex;
  align-items: center;
...
.officer-archive__org-select {
  width: 220px;
  min-width: 160px;
}

.officer-archive__police-select {
  width: 260px;
  min-width: 180px;
}

.officer-archive__controls {
  display: flex;
  align-items: center;
  padding: 12px 20px;
  background: $law-bg-card;
