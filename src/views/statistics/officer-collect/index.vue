<template>
  <div class="officer-collect-container">
    <!-- 页面头部：筛选条件 -->
    <div class="officer-collect__header">
      <h2 class="officer-collect__title">警员采集统计</h2>
      <div class="officer-collect__filters">
        <!-- 时间筛选 -->
        <el-date-picker
          v-model="timeRange"
          type="daterange"
        ...
        <treeselect
          ...
          class="officer-collect__org-select"
        />
        <!-- 警员筛选 -->
        <el-select
          v-model="policeIds"
          ...
          size="small"
          class="officer-collect__police-select"
        >
          <el-option
    ...
    <!-- 指标和维度控制栏 -->
    <div class="officer-collect__controls">
...
import { getOfficerCollectStats } from '@/api/evidence/statistics'
...
export default {
  ...
  computed: {
    ...
    chartTitle() {
      ...
      return '警员' + metricLabel + ' TOP10' + dimLabel
    }
  },
  created() {
    // 默认最近30天
  ...
  methods: {
    ...
    handleReset() {
      ...
      this.chartData = { officerNames: [], metric: 'media_count', metricLabel: '媒体数量', dimension: '', series: [] }
      this.error = false
      this.fetchData()
    },
    ...
    async handleExportExcel() {
      const { officerNames, series, metricLabel, dimension } = this.chartData
      if (!officerNames || !officerNames.length) return
      ...
      try {
        ...
        const data = []
        officerNames.forEach((officerName, officerIdx) => {
          if (!dimension || series.length <= 1) {
            const val = series.length > 0 ? series[0].data[officerIdx] : 0
            const displayVal = this.formatValue(val)
            if (dimension) {
              series.forEach(s => {
                const v = s.data[officerIdx]
                data.push([officerName, s.name, this.formatValue(v)])
              })
            } else {
              data.push([officerName, displayVal])
            }
          } else {
            series.forEach(s => {
              const v = s.data[officerIdx]
...
.officer-collect-container {
  padding: 12px;
  background-color: $law-bg-page;
  min-height: calc(100vh - 84px);
}

.officer-collect__header {
  display: flex;
  align-items: center;
...
.officer-collect__org-select {
  width: 220px;
  min-width: 160px;
}

.officer-collect__police-select {
  width: 260px;
  min-width: 180px;
}

.officer-collect__controls {
  display: flex;
  align-items: center;
  padding: 12px 20px;
  background: $law-bg-card;
