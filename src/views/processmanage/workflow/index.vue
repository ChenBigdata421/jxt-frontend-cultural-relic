<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="workflowList" border>
      <el-table-column label="工作流ID" align="center" prop="id" width="280" />
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="8">
              <el-form-item label="状态">
                ...
                <el-tag v-else-if="form.status === 'cancelled'" type="warning">已取消</el-tag>
              </el-form-item>
            </el-col>
            <el-col :span="8">
        ...
        <div class="workflow-view-section">
          ...
          <div class="workflow-graph-container">
            ...
            <div v-else class="steps-flow">
              <div v-for="(step, index) in viewSteps" :key="step.id" class="flow-item">
                <div class="step-card">
                  <div class="step-icon">
                    <i :class="getStepIcon(step.type)"></i>
                  </div>
                  <div class="step-content">
                    <div class="step-type-label">{{ getStepTypeLabel(step.type) }}</div>
                    <div class="step-name">{{ step.name }}</div>
                    <div v-if="step.description" class="step-desc">{{ step.description }}</div>
                  </div>
                </div>
                <div v-if="index < viewSteps.length - 1" class="flow-arrow">
    ...
    <!-- 启动实例对话框 -->
    <el-dialog title="启动工作流实例" :visible.sync="startInstanceOpen" width="800px" append-to-body>
      <el-form ref="startInstanceForm" :model="startInstanceForm" :rules="startInstanceRules" label-width="120px">
        <el-form-item label="工作流名称">
        ...
        <el-form-item label="优先级">
          <el-select v-model="startInstanceForm.priority" placeholder="请选择优先级" clearable style="width: 100%">
    ...
    <!-- 显示工作流JSON对话框 -->
    <el-dialog title="工作流定义 (JSON)" :visible.sync="jsonDialogVisible" width="70%" append-to-body>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" icon="el-icon-document-copy" @click="copyJSON">复制</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        description: [
          { required: true, message: '工作流描述不能为空', trigger: 'blur' }
        ]
      },
      // 启动实例表单参数
      startInstanceForm: {},
  ...
  methods: {
    ...
    /** 查询角色列表 */
    getRoleList() {
      listRole({ pageIndex: 1, pageSize: 1000 }).then(response => {
        if (response.code === 200 && response.data && response.data.list) {
    ...
    getList() {
      ...
      listWorkflows(params).then(response => {
        if (response.code === 200) {
          this.workflowList = response.data.items || []
    ...
    /** 激活按钮操作 */
    handleActivate(row) {
      const message = row.status === 'frozen'
        ? '是否确认激活工作流"' + row.name + '"？激活后将可以启动新实例，但不能再修改工作流定义。'
        : '是否确认激活工作流"' + row.name + '"?'
      ...
      this.$confirm(message, '警告', {
        ...
        return activateWorkflow(row.id)
      }).then(response => {
        if (response.code === 200) {
    ...
    handleFreeze(row) {
      ...
      this.$confirm('冻结工作流前需要确认所有实例都已完成。是否继续检查？', '提示', {
        ...
        if (response.code === 200) {
          if (response.data.can_freeze) {
            ...
            return this.$confirm(
              ...
              '当前运行中的实例：' + (response.data.running_instances || 0) + '个\n' +
              '已完成的实例：' + (response.data.completed_instances || 0) + '个',
              '确认冻结',
              {
              ...
              return freezeWorkflow(row.id)
            })
          } else {
            // 不能冻结
            this.msgError(
              '无法冻结工作流：' + (response.data.reason || '存在未完成的实例') + '\n' +
              '运行中的实例：' + (response.data.running_instances || 0) + '个'
            )
        ...
        if (error !== 'cancel' && error !== 'cannot_freeze' && error !== 'check_failed') {
    ...
    submitStartInstance() {
      this.$refs['startInstanceForm'].validate(valid => {
        if (valid) {
          ...
          if (this.startInstanceForm.candidate_users && this.startInstanceForm.candidate_users.length > 0) {
            inputData._candidate_users = this.startInstanceForm.candidate_users
          }
          if (this.startInstanceForm.candidate_groups && this.startInstanceForm.candidate_groups.length > 0) {
          ...
          startInstance(data).then(response => {
            if (response.code === 200) {
              ...
              this.$router.push({
                ...
                query: { workflowId: this.currentWorkflow.id, workflowName: this.currentWorkflow.name }
              })
            } else {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          ...
          if (this.form.id != undefined) {
            updateWorkflow(this.form.id, data).then(response => {
              if (response.code === 200) {
