<template>
  <div class="app-container">
    ...
    <!-- 任务处理对话框 -->
    <el-dialog :title="'处理任务 - ' + processForm.task_name" :visible.sync="processOpen" width="900px" append-to-body>
      <el-form ref="processForm" :model="processForm" :rules="processRules" label-width="120px">
        <el-row :gutter="20">
          ...
          <el-col :span="8">
            <el-form-item label="优先级">
              ...
              <el-tag v-else-if="processForm.priority === 'medium'" type="warning">中</el-tag>
        ...
        >
          <div style="margin-top: 10px">
            <p><strong>驳回人：</strong>{{ processForm.rejection_info.rejected_by }}</p>
            <p><strong>驳回时间：</strong>{{ processForm.rejection_info.rejected_at }}</p>
            <p><strong>驳回原因：</strong>{{ processForm.rejection_info.rejection_reason }}</p>
          </div>
        </el-alert>

        <!-- 显示所有前序任务的处理历史 -->
        <div v-if="processForm.previous_tasks_history && processForm.previous_tasks_history.length > 0" style="margin-bottom: 20px">
            ...
            >
              ...
              <el-card shadow="hover" style="margin-bottom: 10px">
                <div slot="header" style="display: flex; justify-content: space-between; align-items: center">
                  ...
                  <el-tag
                    :type="taskHistory.result === '通过' || taskHistory.result === '完成' ? 'success' : 'danger'"
                    size="small"
                  >
                    {{ taskHistory.result }}
                  </el-tag>
                ...
                <div v-if="taskHistory.comment" style="margin-bottom: 15px">
                  <div style="color: #909399; font-size: 12px; margin-bottom: 5px">处理意见：</div>
                  <div style="padding: 10px; background-color: #f5f7fa; border-radius: 4px; color: #606266">
                    {{ taskHistory.comment }}
                  </div>
                </div>

                <!-- 表单字段数据 -->
                <div v-if="taskHistory.output && Object.keys(taskHistory.output).length > 0">
                  <div style="color: #909399; font-size: 12px; margin-bottom: 5px">提交信息：</div>
        ...
        <template v-if="processForm.form_fields && processForm.form_fields.length > 0">
          <el-form-item
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listMyTodoTasks(params).then(response => {
      ...
      getTask(row.id).then(response => {
        ...
        if (workflowResponse.code === 200) {
          ...
          if (definition.steps && definition.steps.length > 0) {
            ...
            console.log('当前任务:', task.task_key, '第一个步骤:', firstStepId, '是否是第一个任务:', this.isFirstTask)
          } else {
          ...
          if (task.form_data) {
            try {
              ...
              console.error('解析 form_data 失败:', e, '原始数据:', task.form_data)
            }
          }
          ...
          if (formDataObj && formDataObj.form_fields) {
            ...
            console.log('提取的 form_fields:', formFields)
          } else {
          ...
          if (task.task_data) {
            try {
              ...
              if (taskData.rejected_by) {
                ...
                console.log('发现驳回信息:', rejectionInfo)
              }
              ...
              if (taskData.previous_tasks_history && Array.isArray(taskData.previous_tasks_history)) {
                ...
                console.log('发现前序任务历史记录:', previousTasksHistory)
              }
            } catch (e) {
              console.error('解析任务数据失败:', e, '原始数据:', task.task_data)
              taskData = {}
            }
          }
    ...
    submitApprove() {
      this.$refs['processForm'].validate(valid => {
        if (valid) {
          ...
          approveTask(this.currentTaskId, data).then(response => {
            if (response.code === 200) {
    ...
    submitDelegate() {
      this.$refs['delegateForm'].validate(valid => {
        if (valid) {
          delegateTask(this.currentTaskId, this.delegateForm).then(response => {
            if (response.code === 200) {
    ...
    getFieldType(field) {
      ...
      // 日期类型
      if (fieldLower.includes('date') && !fieldLower.includes('time') && !fieldLower.includes('datetime')) {
        return 'date'
      }

      // 日期时间类型
      if (fieldLower.includes('datetime') || fieldLower.includes('created_at') ||
          fieldLower.includes('updated_at') || fieldLower.includes('deleted_at') ||
      ...
      if (fieldLower.includes('phone') || fieldLower.includes('mobile') ||
          fieldLower.includes('telephone') || fieldLower.includes('tel')) {
        return 'tel'
      }

      // URL类型
      if (fieldLower.includes('url') || fieldLower.includes('website') || fieldLower.includes('link')) {
        return 'url'
      }

      // 数字类型
      if (fieldLower.includes('amount') || fieldLower.includes('price') ||
          fieldLower.includes('quantity') || fieldLower.includes('count') ||
          fieldLower.includes('number') && !fieldLower.includes('phone') ||
          fieldLower.includes('score') || fieldLower.includes('total')) {
        return 'number'
      }

      // 布尔类型
      if (fieldLower.includes('approved') || fieldLower.includes('deleted') ||
          fieldLower.includes('submitted') || fieldLower.includes('completed') ||
          fieldLower.includes('enabled') || fieldLower.includes('active') ||
          fieldLower.includes('verified') || fieldLower.includes('confirmed') ||
          fieldLower.startsWith('is_') || fieldLower.startsWith('has_') || fieldLower.startsWith('can_')) {
        return 'boolean'
      }

      // 文本域类型
      if (fieldLower.includes('description') || fieldLower.includes('reason') ||
          fieldLower.includes('comment') || fieldLower.includes('remark') ||
          fieldLower.includes('content') || fieldLower.includes('note') ||
          fieldLower.includes('opinion') || fieldLower.includes('detail')) {
        return 'textarea'
      }
      ...
      if (formFields && formFields.length > 0) {
        formFields.forEach(field => {
          ...
          this.processRules[`formData.${field}`] = [
            { required: true, message: `${fieldLabel}不能为空`, trigger: 'blur' }
          ]
