...
import { getSystemConfig } from '@/api/wvp/server'
...
export default {
  ...
  mounted() {
    ...
    // Fetch serverId on mount so the status column works immediately
    getSystemConfig().then(res => {
      this.myServerId = res.data.addOn.serverId
    }).catch(() => {})
  },
  destroyed() {
  ...
  methods: {
    ...
    getTooltipContent: async function(deviceId) {
      let result = ''
      await queryDeviceSyncStatus(deviceId)
        .then((res) => {
    ...
    transportChange: function(row) {
      ...
      updateDeviceTransport(row.deviceId, row.streamMode)
    },
    edit: function(row) {
    ...
    subscribeForCatalog: function(data, value) {
      subscribeCatalog(data, value ? 60 : 0)
        .then(() => {
          this.$message.success({
            showClose: true,
    ...
    subscribeForMobilePosition: function(data, value) {
      subscribeMobilePosition(data, value ? 60 : 0, value ? 5 : 0)
        .then(() => {
          this.$message.success({
            showClose: true,
            message: value ? '订阅成功' : '取消订阅成功'
    ...
    subscribeForAlarm: function(data, value) {
      subscribeForAlarm(data, value ? 60 : 0)
        .then(() => {
          this.$message.success({
            showClose: true,
            message: value ? '订阅成功' : '取消订阅成功'
