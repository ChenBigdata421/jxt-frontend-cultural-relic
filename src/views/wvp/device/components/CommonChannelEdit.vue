    ...
    <el-form ref="channelForm" :model="form" :rules="rules" status-icon label-width="160px" class="channel-form" size="medium">
      <div class="form-box">
        ...
        <el-form-item label="行政区域">
          <el-input v-model="form.gbCivilCode" placeholder="请输入行政区域编码" @change="getRegionPaths" />
        ...
        <el-form-item label="父节点编码">
          <el-input v-model="form.gbParentId" placeholder="请输入父节点编码" @change="getPaths" />
      ...
      <div>
        ...
        <el-form-item label="无效原因码">
          <el-input v-model="form.gbErrCode" type="number" placeholder="请输入无效原因码" />
        </el-form-item>
...
import ResetChannel from './resetChannel.vue'
import diff from '@/utils/diff'
import {
  queryOne,
  update,
  add,
  reset as resetChannel,
  getModelList,
  getGroupPath,
  getRegionPath
  ...
  methods: {
    ...
    getRegionPaths: function() {
      ...
      if (this.form.gbCivilCode) {
        getRegionPath(this.form.gbCivilCode)
          .then(response => {
            let data = response.data
            const path = []
            for (let i = 0; i < data.length; i++) {
