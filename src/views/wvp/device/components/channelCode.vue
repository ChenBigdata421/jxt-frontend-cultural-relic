...
export default {
  ...
  data() {
    return {
      ...
      activeKey: '0',
      allVal: [
        { id: [1, 2], meaning: '省级编码', val: '11', type: '中心编码', lock: false },
        { id: [3, 4], meaning: '市级编码', val: '01', type: '中心编码', lock: false },
        { id: [5, 6], meaning: '区级编码', val: '01', type: '中心编码', lock: false },
        { id: [7, 8], meaning: '基层接入单位编码', val: '01', type: '中心编码', lock: false },
        { id: [9, 10], meaning: '行业编码', val: '00', type: '行业编码', lock: false },
        { id: [11, 13], meaning: '类型编码', val: '132', type: '类型编码', lock: false },
        { id: [14], meaning: '网络标识编码', val: '7', type: '网络标识', lock: false },
        { id: [15, 20], meaning: '设备/用户序号', val: '000001', type: '序号', lock: false }
      ],
      regionList: [],
      deviceTypeList: [],
  ...
  methods: {
    ...
    queryIndustryCodeList: function() {
      ...
      getIndustryList()
        .then(response => {
          this.industryCodeTypeList = response.data || []
        })
        .catch((error) => {
          this.$message.error({
            showClose: true,
    ...
    queryNetworkIdentificationTypeList: function() {
      ...
      getNetworkIdentificationList()
        .then(response => {
          this.networkIdentificationTypeList = response.data || []
        })
        .catch((error) => {
          this.$message.error({
            showClose: true,
