        ...
        <div v-if="currentSelectedIncident" class="media-relations-section">
          ...
          >
            ...
            <el-table-column prop="mediaCate" label="媒体类别" width="100" align="center">
              <template slot-scope="scope">
                {{ formatMediaCate(scope.row.mediaCate) }}
              </template>
            </el-table-column>
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      ...
      if (selection.length === 1) {
        this.currentSelectedIncident = selection[0]
        this.loadMediaRelations(selection[0].id)
      } else {
        this.currentSelectedIncident = null
        this.mediaRelationsList = []
      }
    ...
    /** 格式化媒体类别 */
    formatMediaCate(mediaCate) {
      const cateMap = {
        0: '照片',
        1: '音频',
        2: '视频'
      }
      return cateMap[mediaCate] || '未知'
    }
  }
}
</script>
