...
import { formatFileSize } from '@/utils/dashboard'
    ...
    updateChart() {
      ...
      const option = {
        tooltip: {
          trigger: 'item',
          formatter(p) {
            return p.name + '：<b>' + formatFileSize(p.value) + '</b>（' + p.percent + '%）'
          }
        },
        legend: {
          orient: 'vertical',
          ...
          formatter(name) {
            ...
            return name + '  ' + formatFileSize(item.value) + '  ' + pct + '%'
          }
        },
        series: [{
          type: 'pie',
          ...
          label: {
            ...
            position: 'center',
            formatter() {
              return '{total|' + formatFileSize(total) + '}\n{label|总存储}'
            },
            rich: {
              total: {
                fontSize: 18,
